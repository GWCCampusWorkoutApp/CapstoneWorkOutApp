...
class LegsAndGlutesViewController: UIViewController {
    ...
    @IBAction func legsAndGlutesWorkout(_ sender: Any) {
        recommendRandomly()
    }
    func recommendRandomly() {
    
        var recommendations: [String] = ["Lie on your back with your \rknees bent and soles of your feet on the floor. Extend one leg. On your exhale, squeeze your glutes, and push your hips up toward the ceiling as high as you can. Pause, then lower until your butt hovers right above the floor, and repeat without touching the ground to complete one rep."
        ...
        let titleRec: [String] = ["Single Leg Glute Bridge", "Hydrants with Leg Extensions", "Rainbows", "Bear Plank Leg Lifts", "Walking Lunges (with weights)", "Football Squats", "Diagonal Squats", "Single-Leg Sit to Stand", "Bottoms Up Lunge Crossover", "Rear Foot Elevated Split Squat", "Sumo Squats."]
        
            let length = recommendations.count
            let randomIndex = Int(arc4random_uniform(UInt32(length)))
            hydrantsWithLegExtensions.text =  recommendations[randomIndex]
        titleLabel.text = titleRec[randomIndex]
        }

    
    @IBOutlet weak var hydrantsWithLegExtensions: UITextView!
